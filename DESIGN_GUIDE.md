...
> 아래는 **기본(default) 컨셉** 값. `내 정보 > 화면 컨셉`에서 프리미엄으로 전환하면 `tokens.css`의
> `:root[data-concept="premium"]`가 이 값들을 덮어쓴다. 새 UI 작성 시 아래 px/rgba를 직접 쓰지 말고
> 반드시 토큰(`var(--radius-lg)` 등)으로 참조해야 컨셉 전환이 정상 동작한다.
